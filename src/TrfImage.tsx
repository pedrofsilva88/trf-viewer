...
const getPreRenderedCanvas = (imageProps: TrfImageProps) => {
  ...
  if (!canvas) {
    ...
    const resultRows: { width: number; height: number; mask_color: string; data: Uint8Array }[] = imageProps.db.exec({
      sql: `SELECT width, height, mask_color, data from image where id=${imageProps.id}`,
    ...
    if (resultRows.length > 0) {
      ...
      if (imageInfo.width > 0 && imageInfo.height > 0 && imageInfo.data.length === 3 * imageInfo.height * imageInfo.width) {
        ...
        const [mR, mG, mB] = srcMaskColor.startsWith('#')
          ? [
              Number.parseInt(srcMaskColor.slice(1, 3), 16),
              Number.parseInt(srcMaskColor.slice(3, 5), 16),
              Number.parseInt(srcMaskColor.slice(5, 7), 16),
            ]
          : [255, 255, 255]
