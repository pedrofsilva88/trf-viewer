...
export const TrfPkgSummaryView = (props: TrfPkgSummaryViewProps) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const newEntityTables: TableEntity[] = []
      {
        const infoTable: TableEntity = {
          ...
          data: [
            {
              attr: 'result:',
              value: selected.result === 'NONE' && selected.children.length === 0 ? '(SKIPPED)' : selected.result,
            },
      ...
      }
      {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const resultRows: any[] = trf.db.exec({
          sql: `SELECT * from attribute where reportitem_id = ${selected.id}; `,
          returnValue: 'resultRows',
          rowMode: 'object',
        })
      ...
      {
        const resultRows: { name: string; value: string; description: string; origin: string }[] = trf.db.exec({
      ...
      }
      {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const resultRows: any[] = trf.db.exec({
          sql: `SELECT tcf.* from tcf join ctx on tcf.id = ctx.tcf_id join reportitem on reportitem.ctx_id=ctx.id where reportitem.id =${selected.id}; `,
          returnValue: 'resultRows',
          rowMode: 'object',
        })
        ...
        if (resultRows.length === 1) {
          const constsTable: TableEntity = {
            name: 'test config',
            columns: [{ key: 'cat' }, { key: 'name' }, { key: 'value' }],
            data: [
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const rrMappings: any[] = trf.db.exec({
            sql: `SELECT * from tcf_mappingfile where tcf_id = ${resultRows[0].id};`,
            returnValue: 'resultRows',
            rowMode: 'object',
          })
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const rrEcus: any[] = trf.db.exec({
            sql: `SELECT * from tcf_ecu where tcf_id = ${resultRows[0].id};`,
            returnValue: 'resultRows',
            rowMode: 'object',
          })
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const rrBus: any[] = trf.db.exec({
            sql: `SELECT * from tcf_bus where tcf_id = ${resultRows[0].id} order by id;`,
            returnValue: 'resultRows',
            rowMode: 'object',
          })
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const rrConsts: any[] = trf.db.exec({
            sql: `SELECT * from tcf_const where tcf_id = ${resultRows[0].id};`,
            returnValue: 'resultRows',
            rowMode: 'object',
          })
          if (rrConsts.length) {
...

      {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const resultRows: any[] = trf.db.exec({
          sql: `SELECT tbc.* from tbc join ctx on tbc.id = ctx.tbc_id join reportitem on reportitem.ctx_id=ctx.id where reportitem.id =${selected.id}; `,
          returnValue: 'resultRows',
          rowMode: 'object',
        })
        //console.log(`TrfPkgSummaryView.useEffect[selected] got ${resultRows.length} tbc rows`)
        if (resultRows.length === 1) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const resultRows2: any[] = trf.db.exec({
            sql: `SELECT * from tbc_tool where tbc_id = ${resultRows[0].id} order by id; `,
            returnValue: 'resultRows',
            rowMode: 'object',
          })
          //console.log(`TrfPkgSummaryView.useEffect[selected] got ${resultRows2.length} tbc_tool rows`)
          const data = constsTable.data
          for (const tool of resultRows2 as {
            name: string
            version: string
            status: string
            location: string
  ...
  const panels = useMemo(() => {
    ...
    function CustomTabPanel(props: TabPanelProps) {
      ...
      return (
        <div role='tabpanel' hidden={value !== index} id={`tabpanel_${index} `} {...other}>
          {value === index && <div>{children}</div>}
        </div>
      )
    }

    return entityTables.slice(1).map((et, idx) => (
      <CustomTabPanel key={`tabpanel_${et.name || 'n'}`} value={visibleTab} index={idx}>
  ...
  return (
    <div data-testid='trfPkgSummaryView' className={`trfPkgSummaryView${prefersDarkMode ? ' rs-theme-dark' : ''} `}>
      <div style={{ width: '100%', textAlign: 'center', fontSize: '1rem' }}>{selected.name || selected.label}</div>
      {selected.timestamp && (
        <div style={{ width: '100%', textAlign: 'center', fontSize: '0.7rem' }}>{new Date(selected.timestamp * 1000).toString()}</div>
      )}
