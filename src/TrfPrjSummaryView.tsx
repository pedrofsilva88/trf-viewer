...
export const TrfPrjSummaryView = (props: TrfPrjSummaryViewProps) => {
    ...
    try {
      ...
      }
      {
        ...
        const resultRows: any[] = trf.db.exec({
          sql: `SELECT * from entity where reportitem_id=${selected.id};`,
  ...
  return (
    <div className={`trfPrjSummaryView${prefersDarkMode ? ' rs-theme-dark' : ''} `}>
      <Container vertical={true}>
        <Section
          minSize={100}
          defaultSize={sectionHeight.current}
        ...
        >
          <div style={{ display: 'flex', flexFlow: 'column', height: '100%' }}>
            <div style={{ flex: '0 1 auto' }}>
              <div style={{ width: '100%', textAlign: 'right', fontSize: '0.6rem' }}>{`${
                (trf.dbInfo.app_name || '') + `:${trf.dbInfo.app_version || ''}`
              }`}</div>
              <div style={{ width: '100%', textAlign: 'center', fontSize: '1rem' }}>{`Summary: ${selected.name || selected.label}`}</div>
              {selected.timestamp && (
                <div style={{ width: '100%', textAlign: 'center', fontSize: '0.7rem' }}>
                  {new Date(selected.timestamp * 1000).toString()}
                </div>
              )}
              <Grid container>
                <Grid>
                  {entityTables.length > 0 && (
                    <div style={{ padding: '0px 0px 4px 0px' }}>{tableFromTableEntity(entityTables[0], 'table_info')}</div>
                  )}
                </Grid>
                <Grid xsOffset={'auto'}>
                  {entityTables.length > 1 && (
                    <div style={{ padding: '0px 4px 0px 0px' }}>
                      {tableFromTableEntity(entityTables[1], `table_${entityTables[1].name || 1}`)}
                    </div>
                  )}
                </Grid>
              </Grid>
                ...
                <TrfPrjListView
                  ...
                  items={[
                    ...
                    ...selected.children
                      ...
                      .filter((i) => !(i.result === 'NONE' && i.children.length === 0)),
                  ]}
