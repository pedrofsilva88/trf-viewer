...
export const TrfWorkbenchView = (props: TrfWorkbenchProps) => {
  ...
  useEffect(() => {
    ...
    try {
      // add recordings: to the first level packages
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const rrRecordings: any[] = trf.db.exec({
        sql: `SELECT * from r_reportitem_recording;`,
        returnValue: 'resultRows',
        rowMode: 'object',
      })
              ...
              if (treePkg) {
                ...
                if (!recItem) {
                  recItem = {
                    ...
                    name: 'Recordings',
                    children: [],
                    _internalData: { recordings: [rrRecording] },
                  }
  ...
  if (rootReportItems.length) {
    return (
      <div className='trfWorkbenchView' style={{ display: 'flex', flexDirection: 'column', flex: '1 1 auto', overflowY: 'hidden' }}>
        ...
        >
          <Section
            data-testid='workbench.section.treeView'
            minSize={100}
            defaultSize={section1Width.current}
            maxSize={400}
            onSizeChanged={(curSize) => {
          ...
          <Section minSize={300}>
            {listViewType === ViewType.TestSteps && (
              <div style={{ height: '100%', width: '100%', display: 'grid' }}>
                <Container vertical={true}>
                  <Section
                    minSize={100}
                    defaultSize={section2Height.current}
                    onSizeChanged={(curSize) => {
                  ...
                  >
                    <TrfListView
                      ...
                      key={`listView_${rootReportItems.length}_${treeSelectedItem ? treeSelectedItem.id : 'none'}`}
                      items={rootReportItems}
                      selected={treeSelectedItem}
                      trf={props.trf}
                    />
                  </Section>
                  <Bar size={6} style={{ background: 'currentColor', cursor: 'col-resize' }} />
            ...
            {listViewType === ViewType.PkgSummary && (
              <TrfPkgSummaryView
                items={rootReportItems}
                selected={treeSelectedItem ? treeSelectedItem : rootReportItems[0]}
                trf={props.trf}
              />
            )}
            {listViewType === ViewType.PkgRecordings && (
              <TrfPkgRecordingsView
                items={rootReportItems}
                selected={treeSelectedItem ? treeSelectedItem : rootReportItems[0]}
                trf={props.trf}
                pkgItems={packageItems}
              />
            )}
            {listViewType === ViewType.PrjSummary && (
              <TrfPrjSummaryView
                items={rootReportItems}
                selected={treeSelectedItem ? treeSelectedItem : rootReportItems[0]}
                trf={props.trf}
  ...
  } else return <div>{`reports ${props.trf.fileData.file.name} #${rootReportItems.length} ${JSON.stringify(props.trf.dbInfo)}`}</div>
}
